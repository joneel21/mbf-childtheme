...
if (strlen(implode('', $urls)) != 0) {
    ...
    foreach (array_combine($names, $urls) as $name => $url) {
        ...
        if($mk_options['header_social_networks_style'] == 'simple'){
			$output .= '<li><a class="' . esc_attr( $icon_style_css ) . esc_attr( $name ) . '-hover ' . esc_attr( $header_icon_size ) . '" target="_blank" href="' . esc_url( $url ) . '"><i class="fa fa-'. esc_attr( $name ) .'"><span class="hide">'.esc_attr( $name ).'</span></i></a></li>';
		}
		else{
